...
def findAGW(rdvStore,k,svid):    
    ...
    # return multiple gateways with least logical distance
    return gw                  # Code Change
...
def processPacket(packet):
    ...
    if packettype == RDV_PUBLISH:
        ...
        replypacket = createRDV_REPLY(gw,k,myvid, svid)            # Code change
        ...
        n = len(packet[20:])/4 # Gives number of gateways
        print ' I got rdv_rply with ', n , 'Gateways'
        print "In rdv_reply packettype -- Total Gateways :",n
        # Code added
        for i in range(0, n):
            [gw] = struct.unpack("!I", packet[20+i*4:i*4+24])
            gw_str = bin2str(gw,L)
		...
		continue
            ...
            if(len(routingTable[k]) == 0):
		bucket_info = [nh, gw, getPrefix(myvid,k), 'Yes']
            else:
		bucket_info = [nh, gw, getPrefix(myvid,k), 'No']    
...
for vid in vid2pid: # iterate for every vid in vid2pid list
    ...
    bucket_info = [int(vid,2), int(myvid,2), getPrefix(myvid,dist),'Yes']    
